...
# ==========================================
# SECCIÓN DE ESTILOS Y COLORES (Personalizables)
# ==========================================
COLOR_PRIMARIO = "#0284c7"  # Azul institucional principal
COLOR_FONDO_CAJAS = "#f8fafc"  # Fondo gris/claro para los resultados
COLOR_BORDE = "#cbd5e1"  # Color de los bordes de los recuadros
COLOR_TEXTO_ETIQUETA = "#0369a1"  # Color del texto de la izquierda
COLOR_TEXTO_VALOR = "#1e293b"  # Color del texto de los valores

# Encabezado institucional
st.markdown(
    f"""
    <div style="background-color: {COLOR_PRIMARIO}; padding: 12px; border-radius: 6px; color: white; display: flex; align-items: center;">
...
try:
    ...
    # Coeficientes en métricas compactas
    ...
    # Función para renderizar filas uniformes de resultados
    def fila_resultado(etiqueta, valor):
        st.markdown(
            f"""
            <div style="background-color: {COLOR_FONDO_CAJAS}; padding: 8px 12px; border-radius: 4px; border: 1px solid {COLOR_BORDE}; margin-bottom: 6px; display: flex; justify-content: space-between; align-items: center;">
                <span style="color: {COLOR_TEXTO_ETIQUETA}; font-weight: bold; font-size: 13px;">{etiqueta}</span>
                <span style="color: {COLOR_TEXTO_VALOR}; font-weight: bold; font-size: 13px;">{valor}</span>
            </div>
        """,
            unsafe_allow_html=True,
        )
    ...
    st.markdown(
        f"""
        <div style="background-color: #e0f2fe; padding: 12px; border-radius: 6px; border: 1px solid {COLOR_PRIMARIO}; display: flex; justify-content: space-between; align-items: center;">
            <span style="color: {COLOR_PRIMARIO}; font-weight: bold; font-size: 14px;">TSG por Servicios Generales Total:</span>
            <span style="color: {COLOR_PRIMARIO}; font-weight: bold; font-size: 16px;">{fmt(tasa_total)}</span>
        </div>
    """,
        unsafe_allow_html=True,
    )
